...
def normalize_image(opd, cut, normalize=False, smooth=True):
    ...
    if im_size < 450:
        ...
        cm_x /= im_x.sum()
        cm_x = int(cm_x)
        im[1000 - cm_x:1000 + (im_size - cm_x), :] = image[:, 2:22]
    ...
    if np.amax(liti) == 4088:
        ...
        if len(indices_max) > 1:
            ...
            if (ind_f < 10) or (ind_l > (len(liti) - 10)):
                ...
                popt, pcov = curve_fit(__fit_exp_approx_decay__, x2 + (ind_l - ind_f + 1),
                                       liti[ind_l::].astype("float64"), maxfev=1000)
                exp_decay_extrapolate = __fit_exp_approx_decay__(np.arange(0, ind_l - ind_f + 1), popt[0], popt[1],
                                                                 popt[2])
...
def normalize_lifitime(opd, normalize=True):
    ...
    for i in range(liti.shape[0]):
        if np.amax(liti[i]) == 4088:
            liti[i] = __reconstruct_liti__(liti[i])
...
def spec_correction(opd, normalize=True):
    ...
    temp = np.zeros(spec_2D.shape)
    temp[1::2, :] = spec_2D[::2, :]
    temp[::2, :] = spec_2D[1::2, :]
    spec_2D = np.copy(temp)
